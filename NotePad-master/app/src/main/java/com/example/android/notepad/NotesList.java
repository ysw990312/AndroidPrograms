...
public class NotesList extends ListActivity {
    ...
    private static final String[] PROJECTION = new String[] {
            ...
            NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE  //新增修改时间
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String[] dataColumns = { NotePad.Notes.COLUMN_NAME_TITLE ,NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE} ;  //加一列时间戳

        // The view IDs that will display the cursor columns, initialized to the TextView in
        // noteslist_item.xml
        int[] viewIDs = { android.R.id.text1 ,R.id.text1_time};  //itemview中添加放置时间戳的textview 的id
       ...
       /* *//**
         *  由于SQLight的cursor使用不是很灵活，将cursor中的数据重新全部导出来，重新进行数据的编排和导入
         * 使用简单适配器来填充listview中的数据
         *//*

        List<Map<String,Object>> listitem = new ArrayList<Map<String,Object>>();
        cursor.moveToFirst();   //将cursor移至第一位，一定要做，不然会报数组越界异常！！！
        SimpleDateFormat sdr = new SimpleDateFormat("yyyy年MM月dd日HH:mm:ss");    //设置时间戳格式：yyyy年MM月dd日HH:mm:ss，用于格式化时间戳
        //准备数据源，逐行装填数据，每行装填完移至下一行
        for(int i=0;i<cursor.getCount();i++) {
            Map<String,Object> item = new HashMap<String,Object>();
            item.put(dataColumns[0],cursor.getString(cursor.getColumnIndex("title")));  //获取标题
            int time_stamp = cursor.getInt(cursor.getColumnIndex("modified"));    //获取数据库中的事件戳
            String time_format = sdr.format(new Date(i * 1000L));   //格式化时间戳
            item.put(dataColumns[1],time_format);
            listitem.add(item);
            cursor.moveToNext();   //cursor移至下一行
        }

        SimpleAdapter adapter = new SimpleAdapter(this,listitem,R.layout.noteslist_item,dataColumns,viewIDs);*/
